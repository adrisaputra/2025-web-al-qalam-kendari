            ...
            <div class="card">
                ...
                <div class="card-header border-0 pt-6">
                    ...
                    <div class="card-toolbar">
                        <div class="d-flex justify-content-end" data-kt-achievement-table-toolbar="base">
                            ...
                            <a class="btn btn-success btn-sm me-2 mb-2" data-bs-toggle="modal" data-bs-target="#kt_modal_add_achievement" onClick="clearForm()"><i class="fa fa-plus"></i>Tambah {{ __($title)}}</a>
                        </div>
                    </div>
                ...
                <div class="card-body pt-0">
                    ...
                    <!--begin::Table-->
                    <table class="table table-striped table-rounded border border-gray-300 table-row-bordered table-row-gray-300 gy-2 gs-6" id="achievement-table">
...
<script>
    ...
    $(document).ready(function () {
        table = $('#achievement-table').DataTable({
            processing: true,
            serverSide: true,
			ajax: {
				url: "{{ route('achievement.list', ['url' => Request::segment(1)]) }}",
				type: 'GET',
				dataType: 'json',
			},
            columns: [
        ...
        $('#myForm').submit(function (e) {
            ...
            var id_achievement = $('#id_achievement').val();
            var title = $('#title').val();
            ...
            formData.append('id', id_achievement);
            ...
            var url = "{{ url('/'.Request::segment(1).'/validate') }}";
            $.ajax({
                url: url + "/" + action,
                type: "POST",
                data: formData,
    ...
    function send() {
        ...
        $.ajax({
            url: "{{ url(Request::segment(1).'/store') }}",
            type: "POST",
            data: formData,
            ...
            success: function (response) {
                ...
                $('#kt_modal_add_achievement').modal('hide');
    ...
    function getData(id){
        ...
        var url = "{{ url('/'.Request::segment(1).'/edit') }}";
        $.ajax({
            url: url + "/" + id,
            type: "GET",
            success: function (response) {
                document.getElementById("id_achievement").value = response.data.id;
                ...
                var coverLink = '<br><a href="{{ asset("storage/upload/achievement/") }}/' + response.data.cover + '" class="btn mb-2 mr-1 btn-sm btn-info snackbar-bg-info" target="_blank">Lihat Cover Sebelumnya</a>';
    ...
    function update(id) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $('#kt_modal_add_achievement').modal('hide'); // Tutup modal setelah berhasil memperbarui data
        ...
        new Swal({
            ...
            if (result.isConfirmed) {
                new Swal(
                    ...
                    var url = "{{ url('/'.Request::segment(1).'/delete') }}";
                    $.ajax({
                        url: url + "/" + id,
                        success: function (response) {
